...
export default function StatusCard({ type, score, status, count, title, onClick, ...props }) {
  ...
  if (type === 'score') {
    return (
      <div 
        className="bg-white rounded-lg shadow border p-6 h-full flex flex-col items-center justify-center"
        {...props}
      >
        <div className="relative w-32 h-32 mb-4">
          ...
          <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 120 120">
            ...
            <circle
              ...
              r="50"
              fill="none"
              stroke={score >= 90 ? "#10b981" : score >= 70 ? "#1e40af" : "#ef4444"}
              strokeWidth="8"
              strokeLinecap="round"
        ...
        <div className="mt-2 flex items-center space-x-2">
          <div className={`w-3 h-3 rounded-full ${
            score >= 90 ? 'bg-green-500' : score >= 70 ? 'bg-primary' : 'bg-red-500'
          }`}></div>
  ...
  if (type === 'task') {
    ...
    return (
      <div 
        className={`rounded-lg border-2 p-4 cursor-pointer transition-all duration-200 ${getStatusColor(status)} ${
